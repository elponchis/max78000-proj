...
| ID | 이름 | 주 출처 | 알림 등급 |
|---|---|---|---|
| 0 | `siren` | FSD50K(Siren/Ambulance/Police car/Fire engine), US8K, ESC-50 | 즉시 |
| 1 | `alarm` | FSD50K(Alarm/Alarm clock/Smoke detector), ESC-50 clock_alarm | 즉시 |
| 2 | `glass` | FSD50K(Glass/Shatter), ESC-50 glass_breaking | 즉시 |
| 3 | `scream` | FSD50K(Screaming/Yell/Shout) | 즉시 |
| 4 | `baby_cry` | FSD50K(Crying, sobbing), ESC-50 crying_baby | 기록 |
| 5 | `dog_bark` | FSD50K(Bark/Dog), US8K, ESC-50 | 기록 |
| 6 | `background` | 위 6종 외 전부 + LibriSpeech + MSnoise + US8K 나머지 | — |

> ⚠️ **위 표는 아직 미확정이다.** 2026-09-10 라벨 실측 결과 일부 라벨이 실재하지
> 않고 수량도 예상과 다르다. 확정 전까지 이 표를 근거로 모델·데이터로더를
> 작성하지 말 것. 실측 결과와 미결 쟁점은 **`docs/results/vocabulary-check.md`** 참조.
>
> 요약: `Alarm_clock` / `Smoke_detector_and_smoke_alarm` / 세부 siren 라벨 **부재**.
> siren 128클립·baby_cry 136클립으로 **심각 부족**, scream은 703클립으로 안전
> (리스크 예상이 역전됨). `Alarm`은 전화벨·차량경적 오염 36.7%/9.8%.

- FSD50K `vocabulary.csv`로 라벨명 실재 여부를 **먼저 확인**할 것. 특히
  `Smoke detector, smoke alarm`은 200개 안에 없을 수 있음 → 없으면 `alarm`을
  알람시계·일반 경보로 재정의.
  → **확인 완료(2026-09-10): 부재. `Alarm` 상위 라벨만 사용 가능하며 재정의 필요.**
- `scream`은 데이터 품질 리스크가 가장 크다. 쓸 만한 샘플이 300개 미만이면
  **클래스에서 제외**하고 6클래스로 축소한다. 억지로 넣지 말 것.
  → **확인 완료(2026-09-10): 703클립으로 기준 충족. 다만 샘플 청취 미실시.**

---

## 5. 데이터 규칙 (위반 시 논문이 깨짐)

1. **분할은 원본 클립 단위.** 같은 원본 클립에서 뽑은 윈도우가 train/test에
   흩어지면 데이터 누수다. FSD50K의 dev/eval 분할을 그대로 사용한다.
...
3. **다중라벨 → 단일라벨.** 타깃 6클래스 중 정확히 하나만 포함한 클립만 채택.
   둘 이상 포함 시 제외.
...
**1순위 · 모델 규모별 교차점**
백본 채널 수 0.25×/0.5×/1×/2×로 스윕. 전처리가 전체 지연·에너지에서 차지하는 비중
변화를 측정해 **"모델이 이 크기 이하일 때 전처리 위치가 지배적"**이라는 적용 조건 제시.
전처리는 모델 크기와 무관한 고정 비용이므로, 모델이 작을수록 전처리가 지배한다.
→ 사례를 원칙으로 격상. 추가 하드웨어 0, 학습 6~8회.
→ 합성 결과 169KB / 442KB이므로 2× 규모까지 수용 가능 (4×는 불가)
